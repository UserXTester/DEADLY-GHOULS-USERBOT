...
if Var.TG_BOT_USER_NAME_BF_HER is not None and tgbot is not None:
    ...
    async def inline_handler(event):
        ...
        result = None
        query = event.text
        if event.query.user_id == bot.uid and query == "@DeadlyGhouls_Support":
            rev_text = query[::-1]
            ...
            result = builder.article(
                "@DeadlyGhouls_Support",
                text="""**Hey! This is [DeadlyGhouls.](https://t.me/DeadlyGhouls_Support) \nYou can know more about me from the links given below 👇**""",
                buttons=[
                    [
                        custom.Button.url("🔥 CHANNEL 🔥", "https://t.me/DeadlyGhouls_Support"),
                        custom.Button.url(
                    ...
                    [
                        custom.Button.url(
                            "✨ REPO ✨", "https://github.com/DeadlyGhoulsOP/DeadlyGhouls"),
                        custom.Button.url
                    (
    ...
    async def page(event):
        if not event.query.user_id == bot.uid:
            return await event.answer(
                "HELLO THERE. PLEASE MAKE YOUR OWN MAFIABOT AND USE. © DeadlyGhouls ™",
                cache_time=0,
                alert=True,
            )
        ...
        await event.edit(
            f"**Legenday AF** [DeadlyGhouls](https://t.me/DeadlyGhouls_Support) __Working...__\n\n**Number of modules installed :** `{len(CMD_HELP)}`\n**page:** {page + 1}/{veriler[0]}",
            buttons=veriler[1],
            link_preview=False,
        )
    ...
    async def on_plug_in_callback_query_handler(event):
        if event.query.user_id == bot.uid:
            await delete_deadly(event,
              "⚜️DeadlyGhouls Menu Provider Is now Closed⚜️\n\n         **[© DeadlyGhouls ™](t.me/DeadlyGhouls_Support)**", 5, link_preview=False
            )
        else:
