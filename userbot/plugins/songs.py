...
from DeadlyGhouls.utils import admin_cmd, sudo_cmd, edit_or_reply, progress
...
async def nope(official_sameer):
    ...
    troll = await bot.inline_query("iLyricsBot", f"{(deEmojify(deadly))}")
    ...
    async with borg.conversation("@DeezLoadBot") as conv:
        try:
            ...
            await borg.send_file(
                event.chat_id,
                songh,
                caption="🔆**Here's the requested song!**🔆\n`Check out` [DeadlyGhouls](https://t.me/deadly_USERBOT)",
            )
            await event.delete()
        except YouBlockedUserError:
