...
async def _(event):
    ...
    if tgbotusername is not None or deadly_input == "text":
        results = await event.client.inline_query(tgbotusername, "@DeadlyGhouls_Support")
...
async def info(event):
    ...
    if input_str == "text":
        string = (
            "Total {count} commands found in {plugincount} sudo plugins of DeadlyGhouls\n\n"
        )
        deadlycount = 0
        plugincount = 0
        ...
        if len(string) > 4095:
            ...
            reply_text = f"All commands of the DeadlyGhouls are [here]({url})"
