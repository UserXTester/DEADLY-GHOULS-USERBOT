...
DeadlyGhouls = Config.CUSTOM_STICKER_PACK_NAME
...
async def kang(args):
    ...
    if photo:
        ...
        packname = f"HB_{user.username}_DeadlyGhouls_{pack}"
        packnick = (
            f"{DeadlyGhouls} Vol.{pack}"
            if DeadlyGhouls
            else f"@{user.username}'s DeadlyGhouls Vol.{pack}"
        )
        cmd = "/newpack"
        file = io.BytesIO()
