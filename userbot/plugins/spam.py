...
async def bigspam(deadly):
    if not deadly.text[0].isalpha() and deadly.text[0] not in ("/", "#", "@", "!"):
        ...
        DeadlyGhouls_count = int(deadly_msg[9:13])
        deadly_spam = str(deadly.text[13:])
        for i in range(1, DeadlyGhouls_count):
...
CmdHelp("spam").add_command(
  ...
  "bigspam", "<count> <text>", "Sends the text 'X' number of times. This what DeadlyGhouls iz known for. The Best BigSpam Ever", ".bigspam 5000 Hello"
).add()
