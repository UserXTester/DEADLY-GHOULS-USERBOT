...
# By @Surv_ivor
# fixed some issues by @OFFICIAL_SAMEER
...
async def _(event):
    if event.fwd_from:
        return
    await event.edit(f"Starting superfban By [{DEFAULTUSER}](tg://user?id={deadly}) 😈")
    fedList = []
    if event.reply_to_msg_id:
        previous_message = await event.get_reply_message()
    ...
    if not len(fedList):
        ...
        if len(fedList) == 0:
            await event.edit("Something went wrong.")
            return
    await event.edit(f"Fbaning in {len(fedList)} feds by [{DEFAULTUSER}](tg://user?id={deadly}) 😈")
    try:
...
async def _(event):
    if event.fwd_from:
        return
    await event.edit(f"Starting a superunfban by [{DEFAULTUSER}](tg://user?id={deadly}) ⚡")
    if event.reply_to_msg_id:
        previous_message = await event.get_reply_message()
        FBAN = previous_message.sender_id
    else:
    ...
    await event.edit(f"UnFbaning in {len(fedList)} feds by [{DEFAULTUSER}](tg://user?id={deadly}) ⚡")
    try:
        await event.client.send_message(chat, f"/start")
    except:
        await event.edit("FBAN_GROUP_ID is incorrect.")
