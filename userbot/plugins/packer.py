...
@DeadlyGhouls.on(admin_cmd(pattern=r"unpack", outgoing=True))
@DeadlyGhouls.on(sudo_cmd(pattern=r"unpack"))
async def _(event):
...
@DeadlyGhouls.on(admin_cmd(pattern="repack ?(.*)", outgoing=True))
@DeadlyGhouls.on(sudo_cmd(pattern="repack ?(.*)", allow_sudo=True))
async def _(event):
